...
extension NotificationsViewController  {
    func bindNotificationForUser(){
        if LoginKey.shared.role.uppercased() == "Shop".uppercased(){
            ...
            let userNotifDisposable = clubsNotificationObs.bind(to: NotifTableView!.rx.items(cellIdentifier: "ShopNotifCell")) { row, aShopNotif, cell in
                if let aCell = cell as? ShopNotifCell {
                    let model = aShopNotif
    ...
    func bindGeneralNotification(){
        let generalDisposable = generalNotificationObs.bind(to: NotifTableView!.rx.items(cellIdentifier: "ShopNotifCell")) { row, aShopNotif, cell in
            if let aCell = cell as? ShopNotifCell {
                let model = aShopNotif
